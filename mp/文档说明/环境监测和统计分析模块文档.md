...
7.4.1 启动API服务
```bash
python environmental_monitoring_mysql.py --api
```
...
| `/api/environmental-trend` | GET | 获取环境趋势数据 | station_name: 站点名称, days: 天数（默认7） |
| `/api/air-quality` | GET | 获取空气质量数据 | station_name: 站点名称, days: 天数（默认7） |
| `/api/abnormal-data-statistics` | GET | 获取异常数据统计 | days: 天数（默认30） |
...
# 获取环境趋势数据
curl http://localhost:5000/api/environmental-trend?station_name=监测站1&days=7
```
